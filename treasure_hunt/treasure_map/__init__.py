...
def _transform_raw_map(raw_inp: str, n: int = 5) -> typ.Tuple[typ.Tuple[int]]:
    ...
    if len(flat_map) != n * n:
        raise WrongInputDataError(f'Treasure map dimension should be {n}x{n}')
    if any(1 < number // 10 > n or 1 < number % 10 > n for number in flat_map):
...
def get_treasure_map_from_file() -> typ.Tuple[typ.Tuple[int]]:
    """
    Read input file with treasure map data into a n by n tuple

    :return: 2D tuple of ints (n x n)
    """
    with open(TREASURE_MAP_INP_PATH, 'r', encoding='utf-8') as treasure_map_f:
        raw_map = treasure_map_f.read()
        return _transform_raw_map(raw_map)
